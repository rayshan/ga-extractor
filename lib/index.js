(function() {
  var GaExtractor, Promise, RateLimiter, gApi, gaMaxRowsPerRequest, gaRateLimiter, gp12, p;
  ...
  GaExtractor = (function() {
    function GaExtractor(config) {
      ...
      this.ga = Promise.promisifyAll(gApi.analytics({
        version: 'v3',
        params: {
        ...
        return function(resolve, reject) {
          ...
          _convertKey = new Promise(function(_resolve) {
            if (_this.config.keyPath) {
              ...
              return _resolve(_this.config.keyContent);
            }
          });
          return _convertKey.then(function(keyContent) {
            var _authClient;
            _authClient = new gApi.auth.JWT(_this.config.clientEmail, null, keyContent, "https://www.googleapis.com/auth/analytics.readonly", _this.impersonatedUser);
